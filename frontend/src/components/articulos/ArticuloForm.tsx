          ...
          <div key={name} className="flex flex-col">
            ...
            <Controller
              ...
              render={({ field }) => {
                ...
                if (name === 'precioUnitarioSinIgv' && value) {
                  const stringValue = String(value);
                  const numericValue = parseFloat(stringValue.replace(/[^0-9.-]/g, ''));
                  const formattedValue = isNaN(numericValue) ? stringValue : numericValue.toLocaleString('es-PE');
                  ...
                  return (
                    <div className="relative">
                      ...
                      <input
                        ...
                        id={name}
                        type="text"
                        className="px-8 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary"
                        value={formattedValue}
                        onChange={(e) => {
                          const numericValue = parseFloat(e.target.value.replace(/[^0-9.-]/g, ''));
                          field.onChange(isNaN(numericValue) ? '' : numericValue.toString());
                        }}
                      />
                    </div>
                  );
