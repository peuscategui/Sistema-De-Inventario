...
async function updateInventoryCorrect() {
  try {
    ...
    for (const [index, line] of dataLines.entries()) {
      try {
        ...
        const empleadoId = values[27] ? parseInt(values[27]) : null; // Corregido: columna 27 (índice 27)
        ...
        if (empleadoId && !isNaN(empleadoId)) {
          ...
          console.log(`⚠️ Sin empleadoId válido: ${codigoEFC} (valor: "${values[27]}")`);
        }
