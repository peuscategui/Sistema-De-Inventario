...
async function cargarInventoryProduccion() {
    ...
    try {
        ...
        if (!tableCheck.rows[0].exists) {
            throw new Error('La tabla inventory no existe en la base de datos');
        }
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            try {
                ...
                // Mapear los datos según la estructura de la tabla
                const insertQuery = `
                ...
                const values = [
                    ...
                    columns[24] ? parseFloat(columns[24]) : 0,  // precioUnitarioSinIgv
                    null,  // fecha_baja (no hay datos en el CSV)
                    null,  // motivo_baja (no hay datos en el CSV)
                    columns[26] ? parseInt(columns[26]) : null, // clasificacionId (saltando columna 25 extra)
                    columns[27] ? parseInt(columns[27]) : null  // empleadoId
                ];
