  ...
  try {
    ...
    for (const [index, line] of dataLines.entries()) {
      try {
        ...
        const { rows } = await client.query('SELECT id FROM empleado WHERE id = $1', [parseInt(values[0])]);
        
        if (rows.length > 0) {
          console.log(`⏭️ Saltando empleado ${values[0]}: ya existe`);
          continue;
        }
        ...
        successful++;
        console.log(`✅ Empleado ${values[0]} cargado: ${values[1]}`);
