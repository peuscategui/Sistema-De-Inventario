...
// Función para limpiar una línea CSV eliminando espacios problemáticos
function limpiarLineaCSV(line) {
    // Dividir por comas y limpiar cada campo
    const campos = line.split(',').map(campo => {
        // Eliminar espacios al inicio y final
        let limpio = campo.trim();
        // Eliminar espacios múltiples internos (excepto si está entre comillas)
...
async function limpiarCSVYCargar() {
    ...
    try {
        ...
        const archivoLimpio = path.join(__dirname, '05_inventory_limpio.csv');
        const lineasLimpias = lines.map(line => limpiarLineaCSV(line));
        fs.writeFileSync(archivoLimpio, lineasLimpias.join('\n'));
        console.log('✅ Archivo CSV limpiado guardado como 05_inventory_limpio.csv');
        ...
        for (let i = 1; i < lineasLimpias.length; i++) {
            ...
            try {
                // Dividir por comas (ahora debería estar limpio)
                const columns = line.split(',').map(col => col.trim());
                
                // Verificar que tenemos exactamente 27 columnas del CSV
