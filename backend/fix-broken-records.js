...
async function fixBrokenRecords() {
  try {
    console.log('🔄 Conectando a la base de datos...');
    await client.connect();
    console.log('✅ Conexión exitosa');

    // Datos corregidos para los registros fallidos
    const brokenRecords = [
      {
    ...
    console.log(`\n🔧 Corrigiendo ${brokenRecords.length} registros fallidos...`);
    ...
    for (const record of brokenRecords) {
      try {
        ...
        console.log(`✅ ${record.codigoEFC} corregido - ${record.modelo} - empleadoId: ${record.empleadoId}`);

      } catch (error) {
        console.error(`❌ Error corrigiendo ${record.codigoEFC}: ${error.message}`);
      }
    }
