...
async function updateInventoryFinal() {
  try {
    ...
    for (const [index, line] of dataLines.entries()) {
      try {
        ...
        // El empleadoId está en la última columna (índice values.length - 1)
        ...
        if (empleadoId && !isNaN(empleadoId)) {
          ...
          console.log(`⚠️ Sin empleadoId válido: ${codigoEFC} (valor: "${values[values.length - 1]}")`);
          noEmpleadoId++;
        }
    ...
    console.log(`⚠️ Sin empleadoId válido: ${noEmpleadoId}`);
