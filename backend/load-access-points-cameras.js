...
async function loadAccessPointsCameras() {
  try {
    ...
    for (const [index, line] of dataLines.entries()) {
      try {
        ...
        } else if (codigoEFC.startsWith('AP-') || codigoEFC.startsWith('Ap-')) {
          clasificacionId = 16; // Access Point
        } else if (codigoEFC.startsWith('Cam ')) {
          clasificacionId = 29; // Cámara de seguridad
        }
